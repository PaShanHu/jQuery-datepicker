			...
			return this.each(function() {
				...
				calendar.find('.day').click(function(e) {
					if (!$(this).hasClass('disabled')) {
						var selected = $input.data('datePicker').selected;//selected是$input.data('datePicker').selected的引用
						var displayed = $input.data('datePicker').displayed;
						...
						}else if(opts.output==='zh-CN'){
							$input.val($.dateFormatter.toCNDate(selected));
						}else{
							$input.val($.dateFormatter.toENDate(selected));
						}
